...
from django.shortcuts import render, redirect
from .forms import ContactForm  # Ensure your ContactForm is correctly imported
from django.contrib import messages

def home(request):
    if request.method == "POST":
        form = ContactForm(request.POST)  # Bind form data
        if form.is_valid():
    ...
    return render(request, 'home.html', {'form': form})  # Pass the form to the template
